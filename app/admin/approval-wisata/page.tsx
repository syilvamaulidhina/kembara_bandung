  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {[
          { key: "pending", label: "Pending", count: requests.filter((r) => r.status === "Pending").length, activeClass: "bg-primary text-white" },
          { key: "approved", label: "Disetujui", count: requests.filter((r) => r.status === "Disetujui").length, activeClass: "bg-green-500 text-white" },
          { key: "rejected", label: "Ditolak", count: requests.filter((r) => r.status === "Ditolak").length, activeClass: "bg-red-500 text-white" },
        ].map((tab) => (
          <button
            key={tab.key}
            ...
            {selectedRequest.status === "Pending" && (
              <div className="flex gap-3 mt-6 pt-4 border-t border-gray-100">
                <button
                  ...
                  disabled={processing}
                  className="flex-1 border border-red-200 text-red-600 rounded-xl py-2.5 text-sm font-medium hover:bg-red-50 flex items-center justify-center gap-2 disabled:opacity-60"
                >
                  <X size={16} /> Tolak
                </button>
                <button
